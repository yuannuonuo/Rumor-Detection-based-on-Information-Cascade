...
class DBCN_BiLSTM_Att:
    ...
    modelSavingPath = "./models_Twitter/networks/att_withoutcomments1/"
    ...
    def __init__(self, shuffle_size=20, batch_size=16,
                 ...
                 kernel_width_Residual=8, kernel_num_Residual=4, output_units=2, dropout_rate=0.6,
                 initial_learning_rate=0.000025, epoch_num=20):
        '''
        ...
        self.para_embedding_size = DataPreprocessor4.DataPreprocessor4.para_embedding_size
        self.group_num = DataPreprocessor4.DataPreprocessor4.group_num
    ...
    def __dbcnModel(self, sess):
        ...
        '''
        print("Building DBCN_BiLSTM_Att model!")
        ...
        with tf.name_scope("concat_pool_flat_output"):
            ...
            print("concatbilstm:" + str(concatbilstm))
            concatbilstm = tf.reshape(concatbilstm,[-1,5,100])
            print("reshape:"+str(concatbilstm))
            # Tell TensorFlow we are reusing the scope for the testing
            with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
                cell_fw = contrib.rnn.BasicLSTMCell(50)
                cell_bw = contrib.rnn.BasicLSTMCell(50)
                (outputs, states) = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell_fw, cell_bw=cell_bw,
                                                             inputs=concatbilstm,dtype=tf.float32)

            # rnn_outputs, _ = bi_rnn(BasicLSTMCell(50),
            #                         BasicLSTMCell(50),
            #                         inputs=concatbilstm, dtype=tf.float32)

            fw_outputs, bw_outputs = outputs
            print("fw_outputs "+str(fw_outputs))
            print("bw_outputs " + str(bw_outputs))
            #
            W = tf.Variable(tf.random_normal([50], stddev=0.1))
            H = fw_outputs + bw_outputs  # (batch_size, seq_len, HIDDEN_SIZE)
            M = tf.tanh(H)  # M = tanh(H)  (batch_size, seq_len, HIDDEN_SIZE)
            print("M "+str(M))

            alpha = tf.nn.softmax(tf.reshape(tf.matmul(tf.reshape(M, [-1, 50]),
                                                            tf.reshape(W, [-1, 1])),
                                                  (-1, 5)))  # batch_size x seq_len
            print("alpha "+str(alpha))
            r = tf.tanh(tf.matmul(tf.transpose(H, [0, 2, 1]),
                          tf.reshape(alpha, [-1, 5, 1])))
            r=slim.flatten(r)
            print(r)
            ...
            #                                training=dropout_training_flag)  # dropout.
            dropoutres = tf.layers.dropout(inputs=r, rate=self.dropout_rate,
                                           training=dropout_training_flag)  # dropout.
            # h_drop = tf.nn.dropout(h_star, self.dropout_rate,training=dropout_training_flag)
        ...
        return loss, acc, train_optimization, predictions, labels, alpha
    ...
    def train(self, sess, dataPreprocessor):
        ...
        loss, acc, train_optimization, predictions, labels,alpha = self.__dbcnModel(sess)
        ...
        alphaValueFinal = np.array([0, 0, 0, 0, 0], dtype=np.float32)
        # start training.
        for epoch in range(self.epoch_num):
            ...
            for iteration in range(iterations):
                ...
                _, trainLoss, trainAcc,alphaValue = sess.run([train_optimization, loss, acc,alpha],
                                                  feed_dict={"inputs:0": trainX_batch, "labels:0": trainY_batch,
                                                             "dropout_training_flag:0": True,
                                                             "batchnormalization_training_flag:0": True,
                                                             })
                alphaValue=alphaValue.mean(axis=0)
                alphaValueFinal+=alphaValue
                ...
                # if testAcc > 0.9 and testAcc<0.93:
                    saver.save(sess, DBCN_BiLSTM_Att.modelSavingPath + "dbcnbilstm")
                    previous_best_accuracy = testAcc
                    print("Saving current model!")
            alphaValueFinal=alphaValueFinal.__truediv__(iterations)
            print("Epoch "+str(epoch + 1)+" Alpha:"+str(alphaValueFinal))
    ...
    def test(self, sess, dataPreprocessor):
        ...
        sess.run(init)
        # initialize all variables in tensorflow.
        saver.restore(sess, tf.train.latest_checkpoint(DBCN_BiLSTM_Att.modelSavingPath))
